---
title: "CS229 机器学习完整笔记 — 前六章"
date: 2026-08-14
tags: ["ML", "CS229"]
description: "本笔记涵盖 Stanford CS229 前六章全部内容，适合大厂机器学习/AI 面试复习。每个知识点包含：定义、数学公式、推导要点、面试常见问题。"
---
...
├── 回归基础
│   ├── 线性回归
│   │   ├── 正规方程（闭式解：θ = (X^T X)^(-1) X^T y）
│   │   ├── 概率解释（MLE ↔ 最小二乘，假设误差服从高斯分布）
│   │   └── 过拟合处理 → 岭回归 / Lasso（L2/L1正则）
│   └── 局部加权回归 (LWR)
│       └── 非参数化 → 每次预测保留训练集（面试问：与KNN异同？）
│
├── 分类核心
│   ├── 逻辑回归
│   │   ├── 交叉熵损失（为什么不用均方误差？ → 非凸）
│   │   └── 牛顿法（二阶收敛，比梯度下降快，但Hessian计算贵）
│   └── 感知机
│       └── 收敛定理（数据线性可分时有限步收敛）
│
├── 统一框架
│   └── GLM（广义线性模型）
│       ├── 指数族分布（高斯/Bernoulli/Softmax）
│       ├── 三大假设（给定x，y服从指数族；预测为 E[y|x]；η=θ^T x
│       └── 三种推导 → 线性回归（高斯）/ 逻辑回归（Bernoulli）/ Softmax（多项分布）
│
├── 生成模型
│   ├── GDA（高斯判别分析）
│   │   ├── 高斯假设（多元高斯，共享协方差矩阵）
│   │   └── 与逻辑回归关系（GDA要求更强假设，数据量少时优于LR）
│   └── 朴素贝叶斯
│       ├── 条件独立假设（给定y，特征间相互独立）
│       ├── 拉普拉斯平滑（解决零概率问题）
│       └── 适用场景（文本分类、离散特征）
│
├── 核方法
│   ├── 特征映射 φ(x)（低维→高维，线性不可分→可分）
│   ├── 核函数 K(x,z) = ⟨φ(x), φ(z)⟩（避免显式计算高维特征）
│   ├── 常见核：线性/多项式/RBF/Sigmoid
│   └── Mercer 定理 → 核矩阵正半定（有效核的充要条件）
│
└── SVM（支持向量机）—— 综合顶峰
    ├── 间隔
    │   ├── 函数间隔（依赖尺度，不可用）
    ...
    ├── 对偶
    │   ├── 拉格朗日乘子法（有约束→无约束）
    │   └── KKT条件（互补松弛、梯度为0、原始可行、对偶可行）
    ├── 软间隔
    │   ├── C正则化（平衡间隔与损失）
    │   └── 合页损失（Hinge Loss）→ 支持向量稀疏性来源
    └── SMO（序列最小优化）
        └── 每次选两个变量解析优化（避免大规模QP）
```
